...
As requsted, we used the rvest to extract the html content. however,
instead of extracting via birth year or death year, we chose to look for
elements that are li or ul within the "mw-parser-output" class. this
class seems to contain the main list of the wiki page. then we also
whitelisted elements that contain "," (instead of birth year) so it will
exclude titles.
...
scientist_names <- webpage %>%
  html_nodes(xpath = "//div[contains(@class, 'mw-parser-output')]/ul/li[contains(., ',')]/a[1]") %>%
  html_text()
...
```         
AS we requested to modifIed the names to be valid to extract the URL
by replacing the blank with "\_" and then we succeeded to extract
each URL.
```

```{r, echo = FALSE, warning=FALSE, message=FALSE, results = 'hide'}

wiki_titles <- gsub(" ", "_", scientist_names)

names_urls <- paste0("https://en.wikipedia.org/wiki/", wiki_titles)
...
```{r}
page <- read_html("https://en.wikipedia.org/wiki/Katharine_Bartlett")
  content_text <- page %>%
    html_element(".mw-parser-output") %>%
    html_text2()
  biography_text <- str_split(content_text, "\\nReferences\\n")[[1]][1]
polished_output <- str_trim(biography_text)
```
...
wiki_text_parser <- function(url) {
  tryCatch({
    ...
    content_text <- page %>%
      html_element(".mw-parser-output") %>%
      html_text2()
    start_index <- str_locate(content_text, "From Wikipedia, the free encyclopedia\\n")
    if (!is.na(start_index[1])) {
      biography_start <- start_index[1, "end"] + 1
      content_text <- str_sub(content_text, biography_start)
    }
    ...
    message(paste("Error processing URL:", url, "-", e$message))
    return(NA) # Return NA for failed attempts
  })
}

bio <- sapply(names_urls, wiki_text_parser)
print("success scaped pages: ")
print(length(bio))

scraped_data <- data.frame(name = names(bio), biography = bio)
write.csv(scraped_data, "biographies.csv", row.names = FALSE)
```
